  ...
  module SuperInplaceControls
    ...
    module ControllerMethods
      ...
      def in_place_edit_for(object, attribute, options = {}) 
        define_method("set_#{object}_#{attribute}") do
          ...
          @item.attributes = params[object]
          if @item.save(false)

            if (!options[:list_name].nil? && !options[:list_attribute].nil?)
              final_text = @item.send(options[:list_name]).send(options[:list_attribute]).to_s
            else 
    ...
    module HelperMethods
      ...
      def set_display_text(object, attribute, options)
        ...
        #angela - start
        if (!options[:list_name].nil? && !options[:list_attribute].nil?)
          display_text = object.send(options[:list_name]).send(options[:list_attribute]).to_s
        else 
