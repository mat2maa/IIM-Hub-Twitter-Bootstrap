...
namespace :db do
  task :import_movie_suppliers => :environment do

    FasterCSV.foreach("import/movie_suppliers.csv") do |row|    
      if !row[0].nil?
        
        categories = SupplierCategory.find(:all, :conditions => "name like '%Movie Distributor%'")
    		puts row[0]
